    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['recruit:recommendInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['recruit:recommendInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['recruit:recommendInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['recruit:recommendInfo:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['recruit:recommendInfo:import']"
        >导入</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table :loading="loading" :data="recommendInfoList" @selection-change="handleSelectionChange">
...
import { listRecommendInfo, getRecommendInfo, delRecommendInfo, addRecommendInfo, updateRecommendInfo } from "@/api/recruit/recommendInfo";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/recruit/recommendInfo/importData"
      },
      // 表单校验
      rules: {
        id: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRecommendInfo(this.queryParams).then(response => {
        this.recommendInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getRecommendInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (submitData.id != null) {
            updateRecommendInfo(submitData).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRecommendInfo(submitData).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const recommendInfoIds = row.id || this.ids;
      this.$modal.confirm('是否确认删除用户推荐编号为"' + recommendInfoIds + '"的数据项？').then(function() {
        return delRecommendInfo(recommendInfoIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('recruit/recommendInfo/export', {
        ...this.queryParams
      }, `recommendInfo_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
    ...
    importTemplate() {
      this.download(
        "recruit/recommendInfo/importTemplate",
        {},
        "recommendInfo_template_" + new Date().getTime() + ".xlsx"
      );
    },
    // 文件上传中处理
