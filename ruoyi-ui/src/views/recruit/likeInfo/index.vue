    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['recruit:likeInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['recruit:likeInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['recruit:likeInfo:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['recruit:likeInfo:import']"
        >导入</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table :loading="loading" :data="likeInfoList" @selection-change="handleSelectionChange">
...
import { listLikeInfo, getLikeInfo, delLikeInfo, addLikeInfo, updateLikeInfo } from "@/api/recruit/likeInfo";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/recruit/likeInfo/importData"
      },
      // 表单校验
      rules: {
        id: [
  ...
  methods: {
    ...
    getList() {
      ...
      listLikeInfo(this.queryParams).then(response => {
        this.likeInfoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (submitData.id != null) {
            updateLikeInfo(submitData).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLikeInfo(submitData).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const likeInfoIds = row.id || this.ids;
      this.$modal.confirm('是否确认删除用户点赞编号为"' + likeInfoIds + '"的数据项？').then(function() {
        return delLikeInfo(likeInfoIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('recruit/likeInfo/export', {
        ...this.queryParams
      }, `likeInfo_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
    ...
    importTemplate() {
      this.download(
        "recruit/likeInfo/importTemplate",
        {},
        "likeInfo_template_" + new Date().getTime() + ".xlsx"
      );
    },
    // 文件上传中处理
